function [x,diffusing_species_sum,D,h,alpha_rx,...
...
%     xp(diffusing_species(p),[i i2])=x(diffusing_species(p),[i i2]);
...
% for j=1:6
% % j=1:6;
%     alpha_chem(i+(j-1)*sz) = reaction(i+(j-1)*sz).*x(i+(j-1-1)*sz); %chemical reaction
% end
...
% j=1:6;
    ...
    alpha_chem(ir0+i) = reaction(i+ir0).*x((ir0)+i); %chemical reaction
    alpha_chem(ir0+i2) = reaction(i2+ir0).*x((ir0)+i2); %chemical reaction

...
source = fileread(strcat(chem_RX,".m"));
matched_scripts = scripts(~cellfun('isempty',regexp(source,scripts,'match')));

deps=getScriptDep(strcat(chem_RX,".m"));
init=getInitialized(strcat(chem_RX,".m"));
...
in = fopen(strcat(chem_RX,".m"));



out = fopen(strcat(chem_RX,append,".m"),'wt');

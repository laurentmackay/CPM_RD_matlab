...
if  no_holes
    ...
    elseif shrink
        ...
        if (grow || shrink) && rand<exp(-(dH+Hb)/T) 
            ...
            dist2=sqrt(((i0(vox_trial)-i0).^2)+(j0(vox_trial)-j0).^2)*h;
            pdiff=exp(-(dist2.^2/(4*N*h^2)));
            [fx,fy]=gradient(pdiff);
            jdiff=sqrt(fx.^2+fy.^2);
            plotCellIm(panelD,jdiff,cell_mask,i0,j0);
            colorbar(panelD);
            drawnow;

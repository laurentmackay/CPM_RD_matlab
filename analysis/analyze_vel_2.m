...
figure(2);clf();
    ...
    up_times=cellfun(@(x) cellfun(@(y)  get_up_times(y,thresh), x, 'UniformOutput',0),lambda, 'UniformOutput', 0);
    up_times=[up_times{:}];
    up_times=[up_times{:}];
...
for i=1:length(lambda)
    ...
    up_times=cellfun(@(x) get_up_times(x,1.5), lambda{i},'UniformOutput',0);
